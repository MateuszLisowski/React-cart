...
describe("<CartItem />", () => {
  const wrapper = shallow(
    <CartItem
      index={1}
      product={{ name: "nike", price: 22, chosenSize: 2 }}
      removeItem={() => {}}
    />
  );
  ...
  describe("correctly read and insert to DOM random props", () => {
    ...
    products.forEach(object => {
      const randomWrapper = shallow(
        <CartItem
          index={1}
          product={{
            name: object.name,
            price: object.price,
            chosenSize: object.chosenSize
          }}
          removeItem={() => {}}
        />
      );
      it(`correctly render name(${object.name}) prop`, () => {
        expect(
          randomWrapper
            ...
            .first()
            .html()
        ).toBe(`<p class="cartItemName">${object.name}</p>`);
      });
      it(`correctly render price(${object.price}) prop`, () => {
        expect(
          randomWrapper
            ...
            .childAt(1)
            .html()
        ).toBe(`<p class="cartItemPrice">${object.price} zł</p>`);
      });
      it(`correctly render chosenSize(${object.chosenSize}) prop`, () => {
        expect(
          randomWrapper
            ...
            .childAt(2)
            .html()
        ).toBe(`<p class="cartItemSize">chosen size: ${object.chosenSize}</p>`);
      });
    });
  });
});
